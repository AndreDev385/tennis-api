	...
	async execute(req: AddContestParticipantsDto): Promise<Response> {
		...
		try {
			...
			if (contest.mode.value != GameMode.single) {
				return left(
					Result.fail("Debes inscribir jugadores para un concurso de single"),
				);
			}
			...
			for (const p of req.participants) {
				...
				try {
					...
					try {
						participantAlreadyExist = await this.participantRepo.get({
			...
			const result = await this.registerParticipants.registerBulk(newRecords);
			...
			return right(Result.ok<string[]>(result.errors.map((e) => e.error)));
		} catch (error) {
