    ...
    async execute(request: Req): Promise<Res> {
        ...
        try {
            ...
            if (
                ...
                tournament.rules.matchesPerClash?.value
            ) {
                return left(
                    Result.fail<string>(
                        "La cantidad de partidos no coincide con las reglas"
                    )
                );
            }
            ...
            for (const data of request.matches) {
                ...
                if (combine.isFailure) {
                    return left(
                        Result.fail<string>(`${combine.getErrorValue()}`)
                    );
                }
                ...
                const maybeMatch = TournamentMatch.create({
                    ...
                    sets: Sets.createDefault(
                        tournament.rules.setsQuantity.value
                    ),
                    player1: p1,
                    player2: p2,
                    player3: p3,
