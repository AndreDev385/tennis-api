  ...
  implements UseCase<CreateUserDto, Promise<Response>>
{
  ...
  private async createUser(
    data: UserData,
  ): Promise<Either<Result<string>, Result<void>>> {
    try {
      const userRegisteredByAdmin = await this.repository.get({
        ci: data.ci.value,
      });
      ...
      if (!!userRegisteredByAdmin.email) {
        return left(
          Result.fail<string>(
            `CI: ${data.ci.value} ya se encuentra registrada`,
          ),
        );
      }
      // update user
  ...
  async execute(request: CreateUserDto): Promise<Response> {
    ...
    if (dtoResult.isFailure) {
      return left(Result.fail<string>(dtoResult.getErrorValue())) as Response;
    }
