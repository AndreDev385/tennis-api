...
export class RegisterPlayer
  implements UseCase<RegisterPlayerRequestDto, Response>
{
  constructor(
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          await this.userRepo.save(userAlreadyExist);
      ...
      const result = await this.playerRegisterRepo.register(
        user,
        playerResult.getValue(),
      );

      if (result.isFailure) {
        return left(
          Result.fail<string>("Ha ocurrido un error al registar al jugador"),
        );
      }
