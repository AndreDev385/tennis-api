      ...
      {stats && (
        <Card>
          <Table responsive='sm'>
              ...
              <tr>
                ...
                {playerVsPlayer ? (
                  ...
                  <th className='text-center'>
                    {stats?.player1.name}{' '}
                    {stats?.mode === 'double' ? ' / ' + stats?.player3?.name : ''}
                  </th>
                )}
                {playerVsPlayer ? (
                  <th className='text-center'>
                    {stats?.mode === 'double' ? stats?.player3?.name : stats?.player2}
                  </th>
                ) : (
                  <th className='text-center'>
                    {stats?.player2}{' '}
            ...
            {stats?.mode === 'double' && (
              <CoupleVsTable tracker={stats.tracker} playerVsPlayer={playerVsPlayer} />
            )}

            {stats?.mode === 'single' && (
              <AdvancedTable tracker={stats.tracker} playerVsPlayer={playerVsPlayer} />
            )}
          </Table>
        </Card>
      )}
