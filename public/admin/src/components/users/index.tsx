import { faCheckCircle, faCircle, faCircleNotch, faEllipsisVertical, faSearch, faUser } from "@fortawesome/free-solid-svg-icons";
...
export function UsersTablePage() {
  ...
  const handleListUsers = async () => {
    ...
    const result = await listUsers({ token, query: { isAdmin: 'false', canTrack: 'false' } });
  ...
  const table = filteredUsers.map((item: User) => {
    return (
      <tr key={item.userId}>
        ...
        <td className='text-center'>
          {item.isDeleted ? (
            <FontAwesomeIcon style={{ color: '#fb7676' }} className='circle' icon={faCircle} />
          ) : (
            <FontAwesomeIcon style={{ color: "#0fb85b" }} className='check-circle' icon={faCheckCircle} />
          )}
        </td>
        <td className="text-center">
          <Dropdown>
            <Dropdown.Toggle color="blue" as={Button} id="dropdown-basic" variant="link">
              <FontAwesomeIcon
                color="black"
                className="ellipsis"
  ...
  function render() {
    if (state.loading) {
      return <FontAwesomeIcon className='center mt-5' icon={faCircleNotch} spin />
    }
  ...
  return (<>
    {editUserModal.visible && <EditUserModal user={selectedUser!} dismiss={dismissModal} />}
